    ...
    return (
        <form className={styles.itemBox} key={asset._id} onSubmit={updateAsset.mutate}>
            <div className={styles.infoZone}>
                {isUpdating ? (
                    <input id="name" className={styles.itemBoxInput} placeholder={asset.name} type="text" value={name} onChange={onChangeName} />
                ) : (
                    <div className={styles.assetName}>{asset.name}</div>
                )}
                <div className={asset.amount >= 0 ? styles.amount : styles.amountMinus}>{asset.amount!.toLocaleString()}</div>
                {isUpdating ? (
                    <input id="memo" className={styles.itemBoxInput} placeholder={asset.memo} type="text" value={memo} onChange={onChangeMemo} />
                ) : (
                    <div className={styles.description}>{asset.memo}</div>
                )}
                {!isUpdating && (
